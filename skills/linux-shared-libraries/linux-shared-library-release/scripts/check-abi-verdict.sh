...
if command -v abidiff >/dev/null; then
    ...
    # Member-offset moves and signature changes of existing functions are
    # decisive breaks. abidiff (verified with 2.0/2.4) reports signature
    # changes (parameter added/removed/retyped, return type changed) as
    # "indirect sub-type changes" with only bit 4 set — but a changed
    # signature of an exported function breaks every caller.
    if grep -Eq "offset changed|parameter [0-9]+ of type .* (changed|was added|was removed)|return type changed" "$T/report"; then
        grep -E 'offset changed|size changed|data member|parameter|return type' "$T/report" | head -10 | sed 's/^/  abidiff: /' >&2
        verdict "BREAKING" 1 "abidiff reports offset/signature change(s) in the exported interface (exit $RC); full report: abidiff '$OLD' '$NEW'"
    fi
